...
function readDeveloper(directory) {
	try {
		const content = readFileSync(
			join(directory, ".trellis", ".developer"),
			"utf-8",
		);
...
function buildCompactCurrentState(ctx, platformInput, specIndexPaths) {
	...
	const branch =
		runGit(directory, ["branch", "--show-current"]) || "(detached)";
	const dirtyCount = runGit(directory, ["status", "--porcelain"])
		.split(/\r?\n/)
		.filter((line) => line.trim()).length;
	lines.push(
		`Git: branch ${branch}; ${dirtyCount === 0 ? "clean" : `dirty ${dirtyCount} paths`}.`,
	);
	...
	if (existsSync(tasksDir)) {
		try {
			const activeTasks = readdirSync(tasksDir, { withFileTypes: true }).filter(
				(entry) =>
					entry.isDirectory() &&
					entry.name !== "archive" &&
			...
			lines.push(
				`Active tasks: ${activeTasks.length} total. Use \`python3 ./.trellis/scripts/task.py list --mine\` only if needed.`,
			);
		} catch {
	...
	if (developer !== "(not initialized)" && existsSync(workspaceDir)) {
		try {
			...
			if (journal) {
				...
				const lineCount = readFileSync(journalPath, "utf-8").split(
					/\r?\n/,
				).length;
				lines.push(
					`Journal: .trellis/workspace/${developer}/${journal}, ${lineCount} / 2000 lines.`,
				);
			}
		} catch {
