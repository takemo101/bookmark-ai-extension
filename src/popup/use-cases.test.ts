...
describe("createPopupUseCases", () => {
	it("forwards genuine per-stage save progress in flow order", async () => {
		...
		// Every stage — including `analyzing`/`syncing` — has fired by the time
		// the call resolves: the whole flow runs in the foreground (MIK-021).
	...
	it("emits the same genuine stages for re-analyze", async () => {
		...
		// Seed a saved bookmark to re-analyze; the save completes its analysis in
		// the foreground, so nothing races the re-analyze below.
	...
	it("never emits progress when no observer is passed", async () => {
		...
		expect(result.ok).toBe(true);
		// The whole foreground flow — extraction and analysis — already ran.
